...
def search_word():
    ...
    conn = psycopg2.connect(dbname="postgres", user="postgres", password="Log680968amr", host=db_host, port="5432")
    cursor = conn.cursor()
    ...
    for row in result_DNS:
        ...
        listbox.pack()
        listbox.insert(END, f"Киоск: DNS")
        listbox.insert(END, f"Масть: {row[0]}")
        listbox.insert(END, f"Деньги бумажные: {row[1]}")
    ...
    for row2 in result_Citilink:
        ...
        listbox.pack()
        listbox.insert(END, f"Киоск: Citilink")
        listbox.insert(END, f"Масть: {row2[0]}")
        listbox.insert(END, f"Деньги бумажные: {row2[1]}")
...
if __name__ == "__main__":
    ...
    label = Label(window, text="Кликуха мобилы:")
    label.pack()
    ...
    # Кнопка поиска
    search_button = Button(window, text="Шмонать", command=search_word)
